...
function Logger() {
  ...
  this.activate = function(transportNames, enabled) {
    if (transportNames == null) return;

    if (typeof(enabled) === 'string' || enabled instanceof Array) {
      var _tmp = enabled;
      enabled = transportNames;
      transportNames = _tmp;
    }
  ...
  this.deactivate = function(transportNames) {
    this.activate(transportNames, false);
  }
